-- RETRY: Fix RLS policy syntax (remove NEW.*), and apply full schema/RLS improvements
...
CREATE INDEX IF NOT EXISTS idx_profiles_fb_uid ON public.profiles (fb_uid);
CREATE INDEX IF NOT EXISTS idx_profiles_ig_uid ON public.profiles (ig_uid);
...
WITH CHECK (
  ...
  OR EXISTS (
    ...
    WHERE pr.id = auth.uid()
      AND (
        (platform = 'facebook' AND social_user_id = pr.fb_uid) OR
        (platform = 'instagram' AND social_user_id = pr.ig_uid)
      )
  )
);
